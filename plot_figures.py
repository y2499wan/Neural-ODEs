...
names = ['experiment_node_'+args.dataset+'_v', 'experiment_sonode_conv_'+args.dataset+'_v', 'experiment_anode_'+args.dataset+'_v',\
         'experiment_anode10_'+args.dataset+'_v']
labels =['NODE', 'SONODE', 'ANODE(1)', 'ANODE(10)']
colors = ['#DDAA33', '#004488', '#BB5566', '#2ca02c']
...
def add_bit(x, to_plot, fname, ax):
    ...
    print(names[x], "mean loss", loss_mean[len(loss_mean)-1])    
    print(names[x], "loss std", loss_std[len(loss_std)-1]) 
...
add_bit(3, 1, 'running_epoch_arr.npy', ax1)
rc('font', family='serif')
rc('text', usetex=True)
...
add_bit(3, 2, 'running_epoch_arr.npy', ax2)
rc('font', family='serif')
rc('text', usetex=True)
#plt.legend(loc='lower right', fontsize=12)
plt.xlabel('Epoch', fontsize=16)
